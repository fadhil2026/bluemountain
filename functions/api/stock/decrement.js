...
export async function onRequestPost(context) {
	...
	try {
		...
		if (transaction?.id) {
			await fetch(`${supabaseUrl}/rest/v1/transactions`, {
				...
				body: JSON.stringify({
					...
					payment_status:
						transaction.paymentStatus || transaction.payment_status || "paid",
					paid_amount:
						Number(transaction.paidAmount || transaction.paid_amount) || 0,
					remaining_debt:
						Number(transaction.remainingDebt || transaction.remaining_debt) ||
						0,
					debt_payments:
						transaction.debtPayments || transaction.debt_payments || [],
		...
		return new Response(
			JSON.stringify({
				success: true,
				message: "Stok berhasil diperbarui atomik.",
			}),
			{
				status: 200,
				headers: { "Content-Type": "application/json" },
