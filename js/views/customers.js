...
export const initCustomers = async () => {
	...
	const onResize = () => {
		...
		if (custContainer?.classList.contains("active")) {
			renderCustomers();
		}
	};
...
const getTransactionsForCustomer = (cust, transactions = []) => {
	...
	return transactions
		.filter((t) => {
			if (custId && t.customerId && String(t.customerId) === custId)
				return true;
			if (
				nameKey &&
				t.customerName &&
				t.customerName.trim().toLowerCase() === nameKey
			)
				return true;
			if (
				cleanPhone &&
				t.customerPhone &&
				t.customerPhone.replace(/\D/g, "") === cleanPhone
			)
				return true;
			return false;
		})
...
export const renderCustomers = async () => {
	...
	customers.forEach((c) => {
		...
		const liveSpent = Math.max(
			liveStatsId?.spent || 0,
			liveStatsName?.spent || 0,
		);
	...
	const filtered = customers.filter((c) => {
		const matchCat =
			_currentCategory === "all" || c.category === _currentCategory;
		const matchSearch =
			!_searchQuery ||
...
export const showCustomerModal = (cust = null) => {
  ...
  `;
	...
	document
		.getElementById("modal-cust-close")
		?.addEventListener("click", () => closeModal("modal-cust"));
	document
		.getElementById("cust-cancel-btn")
		?.addEventListener("click", () => closeModal("modal-cust"));

	document
		.getElementById("cust-save-btn")
		?.addEventListener("click", async () => {
			...
			const creditLimit = Math.max(
				0,
				Number(document.getElementById("cf-creditLimit").value) || 0,
			);
			const galonLoaned = Math.max(
				0,
				Number(document.getElementById("cf-galonLoaned").value) || 0,
			);
			const notes = document.getElementById("cf-notes").value.trim();
			const debtInput = document.getElementById("cf-totalDebt");
			const totalDebt = debtInput
				? Math.max(0, Number(debtInput.value) || 0)
				: cust?.totalDebt || 0;
...
export const showCustomer360Drawer = async (cust) => {
  ...
  `;
	...
	document
		...
		?.addEventListener("click", () => closeModal("modal-cust-360"));
	document
		.getElementById("drawer-ok-btn")
		?.addEventListener("click", () => closeModal("modal-cust-360"));
	document
		.getElementById("btn-drawer-pay-debt")
...
export const showPayCustomerDebtModal = async (cust) => {
	const transactions = await getAllTransactions();
	const custTxs = getTransactionsForCustomer(cust, transactions);
	const activeTxs = custTxs.filter(
		(t) => t.paymentMethod === "debt" && (Number(t.remainingDebt) || 0) > 0,
	);
  ...
  `;
	...
	document
		.getElementById("pcd-close-btn")
		?.addEventListener("click", () => closeModal("modal-pay-customer-debt"));
	document
		.getElementById("pcd-cancel-btn")
		?.addEventListener("click", () => closeModal("modal-pay-customer-debt"));
	...
	document
		.getElementById("pcd-submit-btn")
		?.addEventListener("click", async () => {
			const amount = Number(document.getElementById("pcd-amount")?.value) || 0;
			const note =
				document.getElementById("pcd-note")?.value?.trim() ||
				"Pembayaran piutang";

			if (amount <= 0 || amount > totalRemainingDebt) {
				window.showToast?.(
					`Jumlah pembayaran harus antara Rp 1 dan ${formatRupiah(totalRemainingDebt)}`,
					"warning",
				);
				return;
			}
			...
			try {
				...
				const sortedActiveTxs = [...activeTxs].sort(
				...
				for (const tx of sortedActiveTxs) {
					...
					const cicilNote =
						newRem === 0
							...
							: `${note} (Cicilan #${nextCicilNum})`;
					const newPayments = [
						...(tx.debtPayments || []),
						{ date: nowIso, amount: payPortion, note: cicilNote },
					];
				...
				const targetCust =
					freshCusts.find((c) => String(c.id) === String(cust.id)) || cust;
				...
				window.showToast?.(
					`Pembayaran ${formatRupiah(amount)} untuk ${cust.name} berhasil dicatat!`,
					"success",
				);
			} catch (err) {
				console.error("[customer-debt-pay]", err);
				window.showToast?.(
					`Gagal mencatat pembayaran hutang: ${err.message || "Error"}`,
					"error",
				);
			}
		});
