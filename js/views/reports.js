...
import { formatDateTime, monthKey, todayKey } from "../utils/date.js";
...
const renderReportsUI = (txs) => {
	...
	const grossMargin =
		allTotalForProfit > 0
			? ((grossProfit / allTotalForProfit) * 100).toFixed(1)
			: 0;
	...
	const tfToday = todayTxs
		.filter(
			(t) =>
				t.paymentMethod === "transfer" &&
				t.paymentStatus === "transfer_confirmed",
		)
		.reduce((s, t) => s + t.total, 0);
	const tfPending = todayTxs
		.filter(
			(t) =>
				t.paymentMethod === "transfer" &&
	...
	const totalPiutangBelumLunas = txs.reduce(
		(sum, t) => sum + (t.remainingDebt || 0),
		0,
	);
	const _totalTfPendingAll = txs
	...
	let filtered = [...txs];
	if (_tableFilter === "cash")
		filtered = filtered.filter((t) => t.paymentMethod === "cash");
	if (_tableFilter === "transfer")
		filtered = filtered.filter((t) => t.paymentMethod === "transfer");
	if (_tableFilter === "debt")
		filtered = filtered.filter((t) => t.paymentMethod === "debt");
  ...
  `;
	...
	document
		.getElementById("btn-export-pdf-report")
		?.addEventListener("click", () => exportReportPDF(txs, today, month));
	document
		.getElementById("btn-export-csv-report")
		?.addEventListener("click", () => {
			...
			window.showToast?.(
				"✅ Laporan penjualan berhasil diekspor ke Excel/CSV!",
				"success",
			);
		});
	...
	requestAnimationFrame(() =>
		renderCharts(last7, cashToday, tfToday, debtToday, cicilanToday),
	);
};
				...
				datasets: [
					{
						...
						backgroundColor: last7.map((_, i) =>
							i === 6 ? "rgba(37,99,235,0.85)" : "rgba(37,99,235,0.35)",
						),
						borderRadius: 6,
						borderSkipped: false,
					},
...
const exportReportPDF = async (txs, today, month) => {
	try {
		...
		doc.text(s.shopName || "Blue Mountain Refilling Station", pageW / 2, 16, {
			align: "center",
		});
		doc.setFontSize(10);
		doc.setFont("helvetica", "normal");
		doc.text("LAPORAN PENJUALAN & ANALISIS KEUANGAN LENGKAP", pageW / 2, 22, {
			align: "center",
		});
		doc.text(
		...
		const monthTotal = txs
			.filter((t) => t.dateKey?.startsWith(month))
			.reduce((s, t) => s + t.total, 0);
		const allTotal = txs.reduce((s, t) => s + t.total, 0);
		const cashToday = todayTxs
			.filter((t) => t.paymentMethod === "cash")
		...
		const summaryRows = [
			...
			[
				"Kas Masuk Real Hari Ini (Tunai+TF+Cicilan)",
				formatRupiah(kasRealToday),
			],
			["Cicilan Piutang Terkumpul Hari Ini", formatRupiah(cicilanToday)],
			[
				"Total Piutang Belum Lunas (Semua Pelanggan)",
		...
		for (let i = 1; i <= pageCount; i++) {
			...
			doc.setFont("helvetica", "normal");
			doc.text(
				`Hal ${i} dari ${pageCount} — ${s.shopName || "Blue Mountain POS"}`,
				pageW / 2,
		...
		const btn = document.getElementById("btn-export-pdf-report");
		if (btn) {
			btn.textContent = "📄 Export PDF Laporan";
			btn.disabled = false;
		}
	}
};
