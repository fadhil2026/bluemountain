 ...
 * Fixed: bank info reads from store.state.settings, no dynamic imports,
 *        buildReceiptUrl uses getReceiptUrl from printer, preparePrint real function
 */
...
import { generateInvoiceNo, buildReceiptJSON } from '../receipt.js';
...
export const openModal = (html, id = 'generic-modal') => {
  ...
  overlay.innerHTML = `<div class="modal" id="${id}" role="dialog" aria-modal="true" aria-labelledby="modal-title-${id}">${html}</div>`;
...
export const showPaymentModal = (method = 'cash') => {
  ...
  const bankName   = s.bankName   || 'BCA';
  const bankNumber = s.bankNumber || '—';
  const bankHolder = s.bankHolder || s.shopName || 'Blue Mountain Refilling Station';
  ...
  const html = `
    ...
    <div class="modal-body">
      ...
      <div class="payment-method-tabs">
        ...
        <button class="pay-tab ${method === 'debt' ? 'active' : ''}" data-method="debt" style="background:${method === 'debt' ? '#fef3c7' : 'var(--color-warning-bg,#fef9c3)'};border-color:#d97706">
      ...
      <div id="pay-cash-section" style="${method !== 'cash' ? 'display:none' : ''}">
        <div class="input-group">
          ...
          <input type="number" class="input" id="cash-received"
            value="${total}" min="${total}" step="1000"
      ...
      <div id="pay-debt-section" style="${method !== 'debt' ? 'display:none' : ''}">
        ...
        <div class="input-group">
          ...
          <input type="text" class="input" id="debt-customer" placeholder="Nama wajib diisi untuk hutang"
            value="${store.state.customerName || ''}">
        </div>
...
const showSuccessOverlay = (txData) => {
  // Prepare receipt in sessionStorage
  ...
  overlay.innerHTML = `
    ...
    <div class="success-text">
      ...
      <p>${txData.invoiceNo} &bull; ${formatRupiah(txData.total)}</p>
      ${txData.change > 0
    ...
    <div class="success-actions">
      ${canPrint
        ...
        : ''}
      <button class="btn btn--secondary" id="btn-new-tx">🔄 Transaksi Baru</button>
    </div>
