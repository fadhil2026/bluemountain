...
export const openModal = (html, id = "generic-modal", extraClass = "") => {
	...
	const cleanId =
		typeof id === "string" && id.trim() ? id.trim() : "generic-modal";
	const overlay = document.createElement("div");
	overlay.className = "modal-overlay";
	overlay.id = `overlay-${cleanId}`;
	const isWide =
		cleanId === "modal-cust-360" ||
		cleanId === "payment-modal" ||
		extraClass.includes("modal--wide");
	const modalClass = isWide
		? `modal modal--wide ${extraClass}`.trim()
...
export const closeModal = (id = null) => {
	...
	try {
		if (targetId) {
			const el =
				document.getElementById(`overlay-${targetId}`) ||
				document.querySelector(`#overlay-${targetId}`);
			if (el) overlays = [el];
		}
	} catch (_) {}
...
export const showPaymentModal = (method = "cash") => {
  ...
  `;
	...
	setTimeout(() => {
		document
			.getElementById("pay-close-btn")
			?.addEventListener("click", () => closeModal("payment-modal"));
		document
			.getElementById("pay-cancel-btn")
			?.addEventListener("click", () => closeModal("payment-modal"));
		...
		document.querySelectorAll(".pay-tab").forEach((tab) => {
			tab.addEventListener("click", () => {
				for (const t of document.querySelectorAll(".pay-tab")) {
				...
				document.getElementById("pay-cash-section").style.display =
					m === "cash" ? "" : "none";
				document.getElementById("pay-transfer-section").style.display =
					m === "transfer" ? "" : "none";
				document.getElementById("pay-debt-section").style.display =
					m === "debt" ? "" : "none";
		...
		document
			...
			?.addEventListener("click", async () => {
				...
				if (payMethod === "cash") {
					...
					if (paid < total) {
						window.showToast(
							"Jumlah uang tunai kurang dari total tagihan!",
							"warning",
						);
						cashInput?.focus();
						return;
				...
				if (payMethod === "debt") {
					const custName = document
						.getElementById("debt-customer")
						?.value?.trim();
					if (!custName) {
						window.showToast(
							"Nama pelanggan wajib diisi untuk transaksi hutang/cicil!",
							"warning",
						);
					...
					const paidNow = Math.min(
						parseFloat(document.getElementById("debt-paid-now")?.value) || 0,
						total,
					);
					const remaining = total - paidNow;

					// Credit limit guard
					const matchedCust = (store.state.customers || []).find(
						(c) =>
							(c.name || "").trim().toLowerCase() === custName.toLowerCase(),
					);
					if (matchedCust && matchedCust.creditLimit > 0) {
						const projectedDebt =
							(Number(matchedCust.totalDebt) || 0) + remaining;
				...
				if (payMethod === "debt") {
					effectiveCustName =
						document.getElementById("debt-customer")?.value?.trim() ||
				...
				if (payMethod === "cash") {
					...
					const remaining = total - paidNow;
					const status =
						remaining === 0 ? "paid" : paidNow > 0 ? "partial" : "unpaid";
					...
					txData = {
						...
						debtPayments:
							paidNow > 0
								? [{ date: now, amount: paidNow, note: "DP / Uang muka awal" }]
								: [],
						customerId: resolvedCust?.id || null,
						customerName: resolvedCust?.name || effectiveCustName,
						customerPhone: resolvedCust?.phone || "",
				...
				try {
					...
					if (effectiveCustName) {
						if (resolvedCust) {
							resolvedCust.totalOrders =
								(Number(resolvedCust.totalOrders) || 0) + 1;
							resolvedCust.totalSpent =
								(Number(resolvedCust.totalSpent) || 0) + txData.total;
							if (txData.remainingDebt > 0) {
								resolvedCust.totalDebt =
									(Number(resolvedCust.totalDebt) || 0) + txData.remainingDebt;
							}
					...
					window.showToast(
						`Gagal menyimpan transaksi: ${err.message || "Error"}`,
						"error",
					);
					if (btn) {
						btn.disabled = false;
...
const showSuccessOverlay = (txData) => {
	...
	const _printUrl = getPrintSchemeUrl(txData);
...
export const openLoginModal = async ({
  ...
  `;
	...
	const updateDots = () => {
		...
		dots.forEach((dot, idx) => {
			dot.style.background =
				idx < enteredPin.length ? "var(--primary, #2563eb)" : "transparent";
		});
	};
		...
		if (enteredPin.length >= 4) {
			...
			if (authResult.success) {
				...
				const tag = authResult.isServerValidated
					...
					: " (Mode Offline)";
				window.showToast?.(
					`Operator aktif: ${authResult.user.name} (${authResult.user.role})${tag}`,
					"success",
				);
				...
				if (isManual || enteredPin.length >= 6) {
					...
					if (curFail >= 5) {
						...
						if (err)
							err.textContent = "PIN salah 5x! Sistem terkunci 60 detik.";
					} else {
						setLockout(curFail, 0);
						if (err)
							err.textContent = `${authResult.error || "PIN salah!"} (Sisa percobaan: ${5 - curFail})`;
					}
					enteredPin = "";
					updateDots();
					return;
	...
	const bindEvents = () => {
		...
		document
			.getElementById("btn-cancel-login")
