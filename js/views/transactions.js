...
const renderTransactionsUI = (allTxs) => {
	...
	const todayKas = todayTxs.reduce((s, t) => {
		if (t.paymentStatus === "paid" && t.paymentMethod === "cash")
			return s + t.total;
	...
	const startIdx =
		filtered.length === 0 ? 0 : (_currentPage - 1) * PAGE_SIZE + 1;
...
const bindTxEvents = (allTxs) => {
	...
	// Export CSV
	document
		.getElementById("tx-btn-export-csv")
		?.addEventListener("click", () => {
			...
			window.showToast?.(
				"✅ Riwayat transaksi berhasil diekspor ke Excel/CSV!",
				"success",
			);
		});
	...
	document.getElementById("tx-table")?.addEventListener("click", async (e) => {
		...
		if (action === "delete") {
			...
			if (!canDelete(txObj)) {
				window.showToast(
					"Tidak bisa hapus transaksi yang masih ada sisa hutang!",
					"error",
				);
				return;
			}
			if (
				!confirm(
					`Hapus transaksi ${esc(txObj.invoiceNo)}? Tindakan tidak bisa dibatalkan.`,
				)
			)
				return;
			try {
				...
				if (txObj.customerId || txObj.customerName) {
					...
					if (targetCust) {
						targetCust.totalOrders = Math.max(
							0,
							(Number(targetCust.totalOrders) || 1) - 1,
						);
						targetCust.totalSpent = Math.max(
							0,
							(Number(targetCust.totalSpent) || txObj.total) - txObj.total,
						);
						if (
							txObj.paymentMethod === "debt" &&
							(Number(txObj.remainingDebt) || 0) > 0
						) {
							targetCust.totalDebt = Math.max(
								0,
								(Number(targetCust.totalDebt) || 0) -
									Number(txObj.remainingDebt),
							);
						}
...
const showPayDebtModal = (tx) => {
  ...
  `;
	...
	setTimeout(() => {
		document
			.getElementById("debt-x")
			?.addEventListener("click", () => closeModal("debt-modal"));
		document
			.getElementById("debt-cancel")
			?.addEventListener("click", () => closeModal("debt-modal"));
		document
			.getElementById("debt-save")
			?.addEventListener("click", async () => {
				const amount =
					parseFloat(document.getElementById("cicil-amount")?.value) || 0;
				if (amount <= 0 || amount > remaining) {
					window.showToast(
						`Jumlah cicilan harus antara 1 dan ${formatRupiah(remaining)}`,
						"warning",
					);
					return;
				}
				...
				const note =
					document.getElementById("cicil-note")?.value?.trim() || defaultNote;
				...
				try {
					...
					if (tx.customerId || tx.customerName) {
						...
						const targetCust = custs.find(
							(c) =>
								...
								(c.name || "").trim().toLowerCase() ===
									(tx.customerName || "").trim().toLowerCase(),
						);
						if (targetCust) {
							targetCust.totalDebt = Math.max(
								0,
					...
					window.showToast(
						newRemaining === 0
							? "🎉 Hutang LUNAS!"
							: `Cicilan ${formatRupiah(amount)} dicatat`,
						"success",
					);
				} catch (err) {
...
const showTxDetail = (tx) => {
	...
	const _printUrl = getPrintSchemeUrl(tx);
  ...
  `;
	...
	setTimeout(() => {
		document
			.getElementById("td-x")
			?.addEventListener("click", () => closeModal("tx-detail"));
		document
			.getElementById("td-close-btn")
			?.addEventListener("click", () => closeModal("tx-detail"));

		// Direct Universal Thermal Print
		document
			.getElementById("btn-tx-print-direct")
			?.addEventListener("click", () => {
