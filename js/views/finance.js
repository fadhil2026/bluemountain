...
export const initFinance = async () => {
	...
	_unsubTx = store.on("transactions:change", () => {
		if (!_rendering) renderFinance();
	});
	_unsubExp = store.on("expenses:change", () => {
		if (!_rendering) renderFinance();
	});
	_unsubCust = store.on("customers:change", () => {
		if (!_rendering) renderFinance();
	});
	...
	try {
		...
		const [txs, expenses, modalAwalRaw] = await Promise.all([
			store.state.transactions.length
				...
				: getAllTransactions().then((t) => {
						store.setTransactions(t);
						return t;
					}),
		...
		for (const tx of txs) {
			if (
				...
				(tx.paymentStatus === "paid" || !tx.paymentStatus)
			) {
		...
		const totalGalonDipinjam = (store.state.customers || []).reduce(
			(s, c) => s + (Number(c.galonLoaned) || 0),
			0,
		);
		...
		const piutangTotalPages = Math.max(
			1,
			Math.ceil(outstanding.length / PAGE_SIZE),
		);
		if (_piutangPage > piutangTotalPages) _piutangPage = piutangTotalPages;
		const piutangPageItems = outstanding.slice(
			(_piutangPage - 1) * PAGE_SIZE,
			_piutangPage * PAGE_SIZE,
		);
		...
		const expTotalPages = Math.max(
			1,
			Math.ceil(sortedExpenses.length / PAGE_SIZE),
		);
		if (_expensePage > expTotalPages) _expensePage = expTotalPages;
		const expPageItems = sortedExpenses.slice(
			(_expensePage - 1) * PAGE_SIZE,
			_expensePage * PAGE_SIZE,
		);

		// Pagination for Journal (10/page)
		const journalTotalPages = Math.max(
			1,
			Math.ceil(journal.length / PAGE_SIZE),
		);
		if (_journalPage > journalTotalPages) _journalPage = journalTotalPages;
		const journalPageItems = journal.slice(
			(_journalPage - 1) * PAGE_SIZE,
			_journalPage * PAGE_SIZE,
		);
              ...
              ${piutangPageItems
								.map((t) => {
									...
									const percent = Math.min(
										100,
										Math.max(0, Math.round((paidAmt / totalAmt) * 100)),
									);
...
const buildDailyCashFlow = (txs, expenses) => {
	...
	for (const tx of txs) {
		...
		if (
			...
			tx.paymentStatus === "transfer_confirmed"
		) {
		...
		if (tx.paymentMethod === "cash") {
			...
			payments.forEach((p, idx) => {
				...
				entries.push({
					...
					credit: p.amount,
					account: isLunas
						? "[1001] Kas Toko / [1101] Piutang (LUNAS ✅)"
						: "[1001] Kas Toko / [1101] Piutang Usaha",
					type: "kas",
				});
			});
		}
...
const bindFinanceEvents = (txs, _expenses = [], journal = []) => {
	document
		.getElementById("btn-refresh-finance")
		?.addEventListener("click", renderFinance);
	...
	// Export Journal CSV
	document
		.getElementById("btn-export-journal-csv")
		?.addEventListener("click", () => {
			...
			window.showToast?.(
				"✅ Jurnal akuntansi berhasil diekspor ke file CSV/Excel!",
				"success",
			);
		});
	...
	// Set Modal Awal
	document
		.getElementById("btn-set-modal-awal")
		?.addEventListener("click", () => {
    ...
    `;
			...
			setTimeout(() => {
				document
					.getElementById("ma-x")
					?.addEventListener("click", () => closeModal("modal-awal"));
				document
					.getElementById("ma-cancel")
					?.addEventListener("click", () => closeModal("modal-awal"));
				document
					.getElementById("ma-save")
					?.addEventListener("click", async () => {
						const val =
							parseFloat(document.getElementById("modal-awal-input")?.value) ||
							0;
    ...
    `;
		...
		setTimeout(() => {
			document
				.getElementById("exp-x")
				?.addEventListener("click", () => closeModal("expense-modal"));
			document
				.getElementById("exp-cancel")
				?.addEventListener("click", () => closeModal("expense-modal"));
			document
				.getElementById("exp-save")
				?.addEventListener("click", async () => {
					const amount =
						parseFloat(document.getElementById("exp-amount")?.value) || 0;
					const category =
						document.getElementById("exp-category")?.value || "Lainnya";
					const note = document.getElementById("exp-note")?.value?.trim() || "";
					if (amount <= 0) {
						window.showToast("Jumlah harus lebih dari 0!", "warning");
						return;
					}
					const now = new Date().toISOString();
					const exp = {
						date: now,
						dateKey: now.split("T")[0],
						category,
						note,
						amount,
					};
	...
	document
		...
		?.addEventListener("click", async (e) => {
			...
			const txObj = (store.state.transactions || txs).find(
				(t) => String(t.id) === idStr,
			);
			if (!txObj) return;
			...
			if (action === "confirm-transfer") {
				if (
					!confirm(
						`Konfirmasi transfer ${formatRupiah(txObj.total)} dari ${esc(txObj.customerName || "pelanggan")} sudah diterima?`,
					)
				)
					return;
				const updated = {
				...
				try {
					...
					window.showToast("Gagal konfirmasi", "error");
				}
			}
      ...
      `;
				...
				setTimeout(() => {
					document
						.getElementById("mc-x")
						?.addEventListener("click", () => closeModal("mini-cicil"));
					document
						.getElementById("mc-cancel")
						?.addEventListener("click", () => closeModal("mini-cicil"));
					document
						.getElementById("mc-save")
						?.addEventListener("click", async () => {
							const amount =
								parseFloat(document.getElementById("mc-amount")?.value) || 0;
							if (amount <= 0 || amount > remaining) {
								window.showToast("Jumlah tidak valid", "warning");
								return;
							}
							...
							const defaultNote =
								newRemaining === 0
									? `Pelunasan (#${nextCicilNum}/LUNAS ✅)`
									: `Cicilan #${nextCicilNum}`;
							const note =
								document.getElementById("mc-note")?.value?.trim() ||
								defaultNote;

							const newPayments = [
								...(txObj.debtPayments || []),
								{ date: new Date().toISOString(), amount, note },
							];
							const updated = {
								...txObj,
								paidAmount: newPaid,
							...
							try {
								...
								if (txObj.customerId || txObj.customerName) {
									...
									const targetCust = custs.find(
										(c) =>
											(txObj.customerId &&
												String(c.id) === String(txObj.customerId)) ||
											(c.name || "").trim().toLowerCase() ===
												(txObj.customerName || "").trim().toLowerCase(),
									);
									if (targetCust) {
										targetCust.totalDebt = Math.max(
											0,
											(Number(targetCust.totalDebt) || 0) - amount,
										);
								...
								window.showToast(
									newRemaining === 0
										? "🎉 Hutang LUNAS!"
										: `Cicilan #${nextCicilNum} (${formatRupiah(amount)}) dicatat`,
									"success",
								);
							} catch (err) {
								console.error("[cicil]", err);
								window.showToast("Gagal simpan cicilan", "error");
							}
						});
				}, 0);
			}
