...
const bindProductEvents = () => {
	...
	grid?.addEventListener("click", async (e) => {
		...
		if (delBtn) {
			...
			const cleanId = Number.isNaN(Number(idStr)) ? idStr : Number(idStr);
...
const showProductForm = (product = null, allProducts = []) => {
  ...
  `;
	...
	setTimeout(() => {
		document
			.getElementById("pf-close")
			?.addEventListener("click", () => closeModal("product-form"));
		document
			.getElementById("pf-cancel")
			?.addEventListener("click", () => closeModal("product-form"));
		...
		document.getElementById("pf-save")?.addEventListener("click", async () => {
			...
			const category =
				document.getElementById("pf-category")?.value || "Lainnya";
			const emoji = document.getElementById("pf-emoji")?.value || "📦";
			const stock =
				parseInt(document.getElementById("pf-stock")?.value, 10) || 0;
			...
			if (price <= 0) {
				window.showToast("Harga harus lebih dari 0!", "warning");
				return;
			}
...
const showDeleteConfirm = (id) => {
  ...
  `;
	...
	setTimeout(() => {
		document
			.getElementById("dc-close")
			?.addEventListener("click", () => closeModal("delete-confirm"));
		document
