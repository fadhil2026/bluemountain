...
export const isDeviceIsolated = () => {
	try {
		return (
			localStorage.getItem(MASTER_STORE_KEY_STORAGE) === "ISOLATED_SANDBOX"
		);
	} catch (_) {
		return false;
	}
...
export const pushStagedOfflineData = async () => {
	if (!_isServerReachable || isDeviceIsolated()) {
		window.showToast?.(
			"Tidak dapat menyinkronkan: Server belum terjangkau.",
			"warning",
		);
	...
	try {
		...
		const stagedTxs = await db.transactions
			.where("syncStatus")
			.equals("staged_offline")
			.toArray();
		...
		const { error } = await supabase
			.from("transactions")
			.upsert(stagedTxs.map(formatTransactionForCloud));
		...
		window.showToast?.(
			`Sukses menyinkronkan ${stagedTxs.length} transaksi offline ke Cloud!`,
			"success",
		);
		...
		window.showToast?.(
			`Gagal menyinkronkan data offline: ${err.message}`,
			"error",
		);
...
export const formatTransactionForCloud = (tx) => ({
	...
	date_key:
		tx.dateKey ||
		tx.date_key ||
		todayKey(tx.date ? new Date(tx.date) : new Date()),
...
export const formatExpenseForCloud = (exp) => ({
	...
	date_key:
		exp.dateKey ||
		exp.date_key ||
		todayKey(exp.date ? new Date(exp.date) : new Date()),
...
export const formatUserForCloud = (u) => {
	const cleanUsername = String(u.username || "")
		.toLowerCase()
		.trim();
	const userId = u.id
		? String(u.id)
...
export const syncInitialData = async () => {
	...
	const _storeId = getMasterStoreId();
	const supabase = getSupabase();
	_isSyncing = true;
	...
	try {
		...
		try {
			...
			if (!txErr && cloudTxs) {
				const stagedTxs = localTxs.filter(
					(l) => l.syncStatus === "staged_offline",
				);
				if (stagedTxs.length > 0) {
					await supabase
						.from("transactions")
				...
				for (const lt of localTxs) {
					if (
						lt.syncStatus !== "staged_offline" &&
						!cloudTxIdSet.has(String(lt.id))
					) {
		...
		try {
			const [localExps, { data: cloudExps, error: expErr }] = await Promise.all(
				[getAllExpenses(), supabase.from("expenses").select("*")],
			);

			if (!expErr && cloudExps) {
				const stagedExps = localExps.filter(
					(l) => l.syncStatus === "staged_offline",
				);
				if (stagedExps.length > 0) {
					await supabase
						.from("expenses")
						.upsert(stagedExps.map(formatExpenseForCloud));
				...
				for (const le of localExps) {
					if (
						le.syncStatus !== "staged_offline" &&
						!cloudExpIdSet.has(String(le.id))
					) {
		...
		try {
			const [localCusts, { data: cloudCusts, error: custErr }] =
				await Promise.all([
					getAllCustomers(),
				...
				if (payload.new && payload.new.key === `users_roster_${storeId}`) {
					try {
						...
						if (Array.isArray(roster) && roster.length > 0) {
							const rosterUsernames = new Set(
								roster.map((u) => String(u.username).toLowerCase().trim()),
							);
							const localUsers = await db.users.toArray();
							for (const lu of localUsers) {
								if (
									!rosterUsernames.has(String(lu.username).toLowerCase().trim())
								) {
							...
							for (const cu of roster) {
								...
								const existing = await db.users
									.where("username")
									.equalsIgnoreCase(usernameClean)
									.first();
								const userId = cu.id
									? String(cu.id)
									: existing?.id
									...
									isActive:
										cu.is_active !== undefined
											...
											: cu.isActive !== undefined
												...
												: true,
									createdAt:
										cu.created_at || cu.createdAt || new Date().toISOString(),
									updatedAt:
										cu.updated_at || cu.updatedAt || new Date().toISOString(),
								};
...
export const syncAuthoritativeRosterToCache = async () => {
	...
	const cloudUsernames = new Set(
		cloudUsers.map((u) => String(u.username).toLowerCase().trim()),
	);
	const localUsers = await db.users.toArray();
	for (const lu of localUsers) {
...
export const authenticateWithServer = async (usernameOrId, pin) => {
	...
	if (navigator.onLine && !isDeviceIsolated()) {
		try {
			...
			if (cloudUsers && cloudUsers.length > 0) {
				const targetUser = cloudUsers.find(
					(u) =>
	...
	try {
		...
		if (!targetUser) {
			return {
				success: false,
				error: "Perangkat offline dan akun belum tersimpan di cache lokal.",
			};
		}
		...
		return {
			success: false,
			error: `Gagal memvalidasi kredensial: ${err.message}`,
		};
	}
};
