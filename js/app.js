 ...
 */
import { getSetting, openDB, seedDefaultUsers } from "./db.js";
...
const navigateTo = async (viewName) => {
	...
	if (!store.state.currentUser && viewName !== "login") {
		window.showToast?.(
			"Silakan masuk dengan akun operator untuk melanjutkan.",
			"warning",
		);
		viewName = "login";
	}
	...
	if (!store.canAccess(viewName)) {
		window.showToast(
			"Akses dibatasi untuk peran Anda. Silakan hubungi Owner/Supervisor.",
			"warning",
			"Peran Terbatas",
		);
	...
	if (dockContainer) {
		dockContainer.style.display =
			viewName === "login" || !store.state.currentUser ? "none" : "flex";
	}
...
const init = async () => {
	...
	try {
		...
		window.showToast(
			"Database gagal dibuka. Coba reload halaman.",
			"error",
			"Database Error",
		);
		return;
	...
	document
		.getElementById("btn-topbar-logout")
	...
	document
		.getElementById("btn-sync-staged")
	...
	const restoreDockOrder = () => {
		try {
			for (const s of dockEl?.querySelectorAll(".dock-separator") ?? []) {
				s.remove();
			}
	...
	const getMaxScale = () => (isMobile() ? 1.22 : isTablet() ? 1.36 : 1.5);
			...
			if (hasDragTriggered && isDragging) {
				...
				for (let i = 0; i < slotMetrics.length; i++) {
					if (i === 0 && e.clientX < slotMetrics[0].cx) {
						...
						i === slotMetrics.length - 1 &&
						e.clientX >= slotMetrics[i].cx
					) {
						targetSlot = slotMetrics.length - 1;
						break;
					} else if (
						e.clientX >= slotMetrics[i].cx &&
						e.clientX < slotMetrics[i + 1]?.cx
					) {
				...
				currentDropIndex = Math.max(
					0,
					Math.min(dockItems.length - 1, targetSlot),
				);
		...
		const handlePointerEnd = (e) => {
			...
			if (hasDragTriggered && isDragging) {
				...
				if (currentDropIndex !== dragOriginIndex && currentDropIndex >= 0) {
					...
					const newOrder = dockItems
						.map((it) => it.dataset.view)
						.filter(Boolean);
					try {
	...
	const initSwipeNavigation = () => {
		...
		const getDockViewOrder = () => {
			...
			const order = items
				.map((it) => it.dataset.view)
				.filter((v) => Boolean(v) && store.canAccess(v));
	...
	// Realtime App Version Indicator
	const topbarVerEl = document.getElementById("topbar-app-version");
	if (topbarVerEl) {
		const appVer =
			typeof __APP_VERSION__ !== "undefined" ? __APP_VERSION__ : "1.0.0";
		topbarVerEl.textContent = `v${appVer}`;
	}
