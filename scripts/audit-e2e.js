...
import {
	...
	verifyPin,
	verifySessionJWT,
} from "../js/utils/crypto.js";
import { formatRupiah } from "../js/utils/currency.js";
import { monthKey, todayKey } from "../js/utils/date.js";

console.log(
	"🔬 [Military Audit] Memulai pengujian logika bisnis, matematika & akuntansi...",
);
...
assert.strictEqual(
	productStock,
	100,
	"Stok setelah pembatalan harus kembali 100",
);
...
assert.strictEqual(
	token.split(".").length,
	3,
	"JWT harus memiliki 3 segmen terpisah titik",
);
...
assert.strictEqual(
	decoded.username,
	"admin",
	"Klaim username dalam JWT harus sesuai",
);
...
assert.strictEqual(
	wrongKeyResult,
	null,
	"Token dengan secret berbeda harus ditolak",
);
...
assert.strictEqual(
	uuidSet.size,
	1000,
	"1000 UUID berurutan tidak boleh ada tabrakan (zero collision)",
);
...
assert.strictEqual(
	activeItems.length,
	2,
	"Hanya barang tanpa tombstone yang aktif",
);
assert.strictEqual(
	activeItems.map((i) => i.id).join(","),
	"1,3",
	"Barang B terhapus harus tersaring",
);
console.log("     ✓ Filter tombstone soft delete 100% presisi");

// Test 9: Staged Offline Status Transition
console.log(
	"  9. Menguji transisi status sinkronisasi offline (Staged -> Synced)...",
);
const offlineTx = {
	id: generateUUID("tx"),
	total: 50000,
...
assert.strictEqual(
	offlineTx.syncStatus,
	"staged_offline",
	"Transaksi offline harus berstatus staged",
);
offlineTx.syncStatus = "synced";
assert.strictEqual(
	offlineTx.syncStatus,
	"synced",
	"Setelah sinkronisasi harus berstatus synced",
);
console.log("     ✓ Transisi status staged offline lolos");

// Test 10: Multi-Cashier Stock Ledger
console.log(
	"  10. Menguji integritas stok kasir multi-device (atomic bounds)...",
);
let initialStock = 50;
...
assert.strictEqual(
	rejected,
	10,
	"Pesanan 10 harus ditolak karena stok tidak cukup",
);
console.log("     ✓ Validasi stok multi-cashier lolos");

// Test 11: Cloud Roster User Mapping & ID Preservation
console.log(
	"  11. Menguji pemetaan user cloud roster & kebal DataError Dexie...",
);
...
assert.strictEqual(
	mappedUsers.every((u) => typeof u.id === "string" && u.id.length > 0),
	true,
	"Setiap user harus memiliki id string",
);
assert.strictEqual(
	mappedUsers[0].id,
	"usr_admin",
	"ID admin ter-generate konsisten",
);
assert.strictEqual(
	mappedUsers[2].id,
	"usr_custom_99",
	"ID eksisting dipertahankan",
);
console.log(
	"     ✓ Pemetaan user cloud & integritas primary key Dexie 100% aman",
);

// Test 12: Cloud Format DateKey Fallback Resilience (Zero ReferenceError)
console.log(
	"  12. Menguji ketahanan formatTransactionForCloud & formatExpenseForCloud...",
);
...
assert.match(
	txFormattedDateKey,
	/^\d{4}-\d{2}-\d{2}$/,
	"DateKey fallback transaksi harus valid",
);
assert.match(
	expFormattedDateKey,
	/^\d{4}-\d{2}-\d{2}$/,
	"DateKey fallback pengeluaran harus valid",
);
...
const parsedIntResult = parseInt(datasetId, 10);
assert.strictEqual(
	Number.isNaN(parsedIntResult),
	true,
	"parseInt pada UUID harus NaN (bukti bug lama)",
);
const safeIdStr = String(datasetId);
assert.strictEqual(
	safeIdStr,
	datasetId,
	"String(id) mempertahankan UUID murni tanpa korupsi",
);
