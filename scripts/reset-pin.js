...
const SUPABASE_SECRET_KEY = process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_BBEJNs18ooZ-IHRPxJtDUA_KiKLcQ-g';
...
async function runResetPin() {
  console.log(`🔐 [Reset PIN Realtime Engine] Menghubungi Supabase Cloud (${SUPABASE_URL})...`);
  ...
  if (!Array.isArray(roster) || roster.length === 0) {
    console.log('   ℹ️ Roster cloud belum ada, menginisialisasi daftar operator standar...');
    roster = [
      {
        username: 'admin',
    ...
    if (!userFound) {
      console.log(`   ℹ️ Operator @${targetUsername} belum ada di roster, menambahkan operator baru...`);
      roster.push({
        username: targetUsername,
        name: targetUsername === 'admin' ? 'Fadhilah Ramadhan' : (targetUsername.charAt(0).toUpperCase() + targetUsername.slice(1)),
  ...
  if (saveErr) {
    console.error('❌ Gagal menyimpan roster ke Supabase settings:', saveErr.message);
    process.exit(1);
  }
  ...
  console.log('   • Realtime : Seluruh perangkat (Desktop, HP, Tablet) yang sedang online');
  console.log('                akan seketika tersinkronisasi otomatis via WebSocket.');
}
