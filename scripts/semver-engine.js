...
export const analyzeChanges = () => {
  ...
  try {
    ...
    // Ambil diff konten
    diffOutput = execSync('git diff HEAD', { stdio: ['ignore', 'pipe', 'ignore'] }).toString();

    // Ambil pesan commit 3 terakhir untuk evaluasi
    commitMessages = execSync('git log -n 3 --oneline', { stdio: ['ignore', 'pipe', 'ignore'] }).toString();
  } catch (_) {}
  ...
  if (
    ...
    sMajor += 100;
    reasons.push('Terdeteksi indikasi breaking change / migrasi arsitektur mayor');
  }

  // 2. Evaluasi MINOR (Fitur / Modul Baru)
  if (
    /\bfeat(\([^)]+\))?:/i.test(commitMessages) ||
    changedFiles.some(f => f.startsWith('js/views/') && statusIncludesNew(f)) ||
    changedFiles.some(f => f.includes('09_MASTER_TENANT_ISOLATION') || f.includes('semver-engine')) ||
    /master store tenant isolation/i.test(textToScan) ||
    /dual-bridge/i.test(textToScan)
  ) {
    sMinor += 50;
    reasons.push('Terdeteksi penambahan modul baru / arsitektur fitur baru (Tenant Isolation / Dual-Bridge / SemVer Engine)');
  }
  ...
  if (
    ...
    sPatch += 15;
    reasons.push('Terdeteksi perbaikan bug, styling CSS, atau pembaruan dokumentasi');
  }
...
function statusIncludesNew(filePath) {
  try {
    const status = execSync(`git status --porcelain "${filePath}"`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString();
...
export const bumpVersion = (type = 'auto', description = '') => {
  ...
  try {
    state.lastCommitHash = execSync('git rev-parse --short HEAD', { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
  } catch (_) {}
...
if (process.argv[1] && process.argv[1].endsWith('semver-engine.js')) {
  ...
  if (arg === '--analyze') {
    ...
    console.log(`🚀 [SemVer Engine] Versi dinaikkan (${type}) -> v${nextVer}`);
  } else if (arg === '--sync') {
    const state = getSemVerState();
    syncVersionEverywhere(state.currentVersion);
    console.log(`🔄 [SemVer Engine] Berhasil menyinkronkan v${state.currentVersion} ke seluruh berkas.`);
  } else {
