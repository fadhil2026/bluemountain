...
if (!SUPABASE_SECRET_KEY) {
	console.error(
		"❌ SUPABASE_SECRET_KEY tidak ditemukan di .env. Pastikan file .env lokal terkonfigurasi.",
	);
	process.exit(1);
}
...
const testKeys = async () => {
	...
	const { error: pubErr } = await pubClient
		.from("products")
		.select("*")
		.limit(1);
	console.log(
		`  Publishable Key : ${pubErr ? `❌ Gagal: ${pubErr.message}` : "✅ Berfungsi (Read Access OK)"}`,
	);
	...
	const { error: secErr } = await adminClient
		...
		.limit(1);
	console.log(
		`  Secret Admin Key: ${secErr ? `❌ Gagal: ${secErr.message}` : "✅ Berfungsi (Full Admin Bypass OK)"}`,
	);
};
...
switch (command) {
	...
	case "query":
		if (!args[0]) {
			console.error(
				"Harap masukkan nama tabel. Contoh: node scripts/supabase-admin.js query products 5",
			);
		} else {
